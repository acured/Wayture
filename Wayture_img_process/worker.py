"""
Wayture Image Worker — 轮询 Azure Storage Queue 处理图片生成任务
...
from services.executors import (
    execute_album_task,
    execute_gallery_task,
    execute_journal_task,
    execute_postcard_task,
)
...
def log(task_id: str, status: str, desc: str, worker: int = 0):
    total = completed_count + failed_count
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"[{total}] [W{worker}] [{task_id}] [{status}] {desc}  ({ts})")


async def _save_failure_log(task_id, username, task_type, task_data, error, attempts):
    failure = {
        "task_id": task_id,
        "username": username,
        "task_type": task_type,
...
async def _save_memory_from_result(username, task_type, result):
...
async def process_message(msg, worker_id: int = 0) -> None:
    ...
    try:
        ...
        if isinstance(task_data, list):
            log(task_id, "ERROR", "task.json 格式异常", w)
            await _save_failure_log(task_id, username, "unknown", {}, "task.json is a list", 0)
        ...
        log(task_id, "RECV", f"user={username} type={task_type}", w)

        executor = EXECUTORS.get(task_type)
        if not executor:
            log(task_id, "ERROR", f"未知任务类型: {task_type}", w)
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            log(task_id, "PROCESSING", f"第 {attempt}/{MAX_ATTEMPTS} 次尝试 ({task_type})", w)
            start = time.time()
            try:
                ...
                log(task_id, "FAILED", f"第 {attempt}/{MAX_ATTEMPTS} 次失败 ({elapsed:.1f}s)", w)
                if attempt < MAX_ATTEMPTS:
                    log(task_id, f"RETRY {attempt}/{MAX_ATTEMPTS - 1}", f"{RETRY_DELAY}s 后重试...", w)
                    await asyncio.sleep(RETRY_DELAY)
        else:
            failed_count += 1
            log(task_id, "GIVE_UP", f"已重试 {MAX_ATTEMPTS} 次，放弃", w)
        ...
        delete_message(msg)
        completed_count += 1
        log(task_id, "COMPLETED", f"user={username} type={task_type} ({elapsed:.1f}s)", w)

    except Exception:
        failed_count += 1
        log(task_id, "CRASH", f"未捕获异常", w)
        traceback.print_exc()
        try:
...
async def poll_loop():
    print("========================================")
    print("  Wayture Image Worker 启动")
    print(f"  队列: image-tasks  并发: {CONCURRENCY}  重试: {MAX_ATTEMPTS} 次")
    print("========================================")
    ...
    while True:
        ...
        slots = CONCURRENCY - len(active_tasks)
        if slots <= 0:
        ...
        if not messages:
            ...
            if poll_count % 50 == 0:
                ts = datetime.now().strftime("%H:%M:%S")
                print(f"[{completed_count + failed_count}] [--] [IDLE] 轮询 {poll_count} 次 (完成:{completed_count} 失败:{failed_count} 进行中:{len(active_tasks)})  ({ts})")
            await asyncio.sleep(2)
            continue

        poll_count = 0
        for msg in messages:
            worker_seq = (worker_seq % CONCURRENCY) + 1
            wid = worker_seq
            async def _run(m=msg, w=wid):
                async with semaphore:
                    await process_message(m, w)
